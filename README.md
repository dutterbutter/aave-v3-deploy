...
## How to deploy Aave V3 on zkSync network
...
1. **Environment Configuration**:
   - Duplicate the `.env.example` file:
     ...
     cp .env.example .env
     ```
   - Edit `.env` with your details (MNEMONIC, RPC_URL, MARKET_NAME):
     ```
     nano .env
     ```
   Refer to [pool configurations](https://github.com/aave/aave-v3-deploy/blob/09e91b80aff219da80f35a9fc55dafc5d698b574/helpers/market-config-helpers.ts#L95) for valid `MARKET_NAME` values.
...
3. **Compile Contracts**:
   Replace `<network>` with your target network (e.g., `zkSync-goerli`, `zkSync-sepolia`, `zkSyncLocal`, `zkSync`).
   ```
   HARDHAT_NETWORK=zkSyncLocal SKIP_LOAD=true npx hardhat compile
   ```

4. **Deploy Libraries**:
   Follow the prompt to deploy libraries:
   ```
   yarn hardhat deploy-zksync:libraries --compile-all-contracts --private-key <PRIVATE_KEY> --network zkSyncLocal
   ```

   **NOTE:** Make sure to include `--compile-all-contracts`.

5. **Deploy to zkSync**:
   ```
   HARDHAT_NETWORK=zkSyncLocal npx hardhat deploy
   ```
